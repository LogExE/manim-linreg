...
class LinRegIntro(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(g, linreg))

        descr = MarkupText("<b>Условие задачи</b>").move_to(2 * UP)
        given = Tex(r"Даны измерения переменной $x$ (обозн. $x_i$) и целевой\\переменной $y$ (обозн. $y_i$). Между ними, предположительно,\\есть значительная линейная связь.").next_to(
            descr, DOWN, buff=2 * DEFAULT_MOBJECT_TO_MOBJECT_BUFFER)
        task = Tex(
            r"Требуется построить прямую $\hat{y} = a \cdot x + b$,\\проходящую рядом с точками $(x_i, y_i)$ так, чтобы\\значение суммарной ошибки $\sum\limits_{i=1}^{n}(\hat{y_i} - y_i)^2$ было наименьшим.").next_to(given, DOWN)
        self.play(AnimationGroup(Write(descr), Write(
            given), Write(task), lag_ratio=1.25, run_time=15))
        self.wait()
...
class LinRegTask(Scene):
    def construct(self):
        ...
        self.play(Create(uptext), FadeIn(data_table),
                  Create(ax), Create(xylabel))
        ...
        def line1_fun(x): return x / 3 + 4
        line1 = ax.plot(line1_fun, x_range=[0, 15]).set_color(RED)
        text_label = Tex(r'Уравнение прямой\\', r'$y = a \cdot x + b$')
        line1_label = ax.get_graph_label(
            line1, label=text_label, direction=3 * UP + RIGHT)
        ...
        y_hat_text.scale_to_fit_width(
            data_table_mod.width).next_to(data_table_mod, DOWN)
        y_hat_group = VGroup(y_hat_text, data_table_mod.get_rows()[2])
        self.play(Succession(Create(y_hat_text), Indicate(y_hat_group)))
        self.wait()

        task = Tex(
            r'Задача:\\', r'$S(a, b) = \sum\limits_{i=1}^{n}(\hat{y_i} - y_i)^2 \rightarrow \min$')
...
class LinRegMain(Scene):
    def construct(self):
        ...
        self.wait(2)
        expr1 = MathTex(
            r' = \sum\limits_{i=1}^{n}(\hat{y_i} - y_i)^2').next_to(eq1)
        self.play(Write(expr1))
        self.wait(2)
        expr2 = MathTex(
            r' = \sum\limits_{i=1}^{n}(a \cdot x_i + b - y_i)^2').next_to(eq1)
        ...
        self.play(FadeOut(eq1), FadeOut(expr1), FadeOut(
            note1), FadeOut(sys1), FadeOut(sys2))
        self.play(sys3.animate.move_to(2 * UP))
        self.wait()

        note2 = Text('Решим систему методом Крамера:').next_to(
            sys3, DOWN).scale(0.8)
        self.play(Write(note2))
        self.wait()

        vars = MathTex(
            r'a=\dfrac{\Delta_a}{\Delta},\ b=\dfrac{\Delta_b}{\Delta}')
